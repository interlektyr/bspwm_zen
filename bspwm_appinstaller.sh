...
gum spin --spinner pulse --title "Syncing databases" -- wget -q https://aur.archlinux.org/packages.gz -P /$HOME/.config/bspwm/ >/dev/null
...
va=$(cat /$HOME/.config/bspwm/bspwm_applist.txt | fzf --border --border-label=" AppInstaller " --color=label:italic:yellow --color=border:'#1D2021')
